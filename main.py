...
## 输出结构：15章报告（与十五步1:1对齐）

报告必须包含以下15个章节。每个章节标题格式为：
## Chapter N: 中文标题

### Chapter 1: 八字排盘（编码）
四柱表（年/月/日/时干支+藏干+纳音+空亡）、真太阳时校正、五行分布计数
对应十五步：第1步·编码（排八字）

### Chapter 2: 识日主（日主性质与喜忌）
日干十天干性质、《滴天髓》十干论、喻象/性质/喜忌、阳干阴干区分
对应十五步：第2步·识日主

### Chapter 3: 观月令（节气与当令之气）
月支对应节气/季节、分日用事、月令本气是否透干、季月特殊处理
对应十五步：第3步·观月令

### Chapter 4: 日主强弱分析
五行统计、同方/异方加权、当令修正、偏强/偏弱/中和判定
对应十五步：第4步·日主强弱分析

### Chapter 5: 调候分析
《穷通宝鉴》十干配十二月、调候用神是否到位、寒暖燥湿判断
对应十五步：第5步·调候分析

### Chapter 6: 格局分析
取格依据（月支本气→中余气→别干支）、相神、格局成败、格局高低评估
对应十五步：第6步·格局分析

### Chapter 7: 形象分析（清浊/真假/源流/通关）
命局类型（独象/偶象/从象/化象/全象）、清浊、真假、源流、通关
对应十五步：第7步·形象分析

### Chapter 8: 用神取用（全局喜忌）
用神/喜神/忌神/仇神/闲神确定，权衡格局/通关/病药/专旺/调候
对应十五步：第8步·确定全局用神和喜忌
...
### Chapter 10: 疾病提示
五行受克受伤、天干对应脏腑、五行太旺太弱俱病
对应十五步：第10步·论疾病

### Chapter 11: 六亲分析
宫位+十神参合（年=祖上、月=父母兄弟、日支=配偶、时=子女）
对应十五步：第11步·论六亲

### Chapter 12: 财官（富贵贫贱）
官星状态（事业/权力）、财星状态（财富）、财官印食配合
对应十五步：第12步·论财官

### Chapter 13: 大运流年
起运计算、完整大运表（8步×10年，标注十神/格局喜忌/调候喜忌）、当前大运、未来3年流年+凶度评分
对应十五步：第13步·大运流年动态分析

### Chapter 14: 神煞辅助验证与补充推算
神煞（天乙贵人/文昌/驿马/华盖/桃花/羊刃/空亡）、命宫、小限、空亡分析、病药说
对应十五步：第14步·神煞辅助验证 + 补充步骤A-G

### Chapter 15: 综合判断与建议
多角度验证（必须验证已知事实）、四家投票共识表、紫微斗数交叉验证、破局方向、黄金窗口、注意事项、格局总评
对应十五步：第15步·综合判断
